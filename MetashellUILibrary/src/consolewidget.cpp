    ...
    QWidget(parent)
{
    ...
    QFont t_font = QFontDatabase::systemFont(QFontDatabase::FixedFont);
    ...
    m_selection.fromRow = m_selection.fromChar = m_selection.startingChar = m_selection.startingRow = m_selection.toChar = m_selection.toRow = 0;
    ...
    m_showInputLine = false;
    m_scrollDown = false;
    connect(m_promptTimer.get(), SIGNAL(timeout()), this, SLOT(promptTimerTicked()));
}
...
void ConsoleWidget::addAsInput(QString p_text)
{
    m_showInputLine = false;
    addText(m_prompt + p_text);
}
...
void ConsoleWidget::codeCompletionArrived(QStringList p_list)
{
    if(p_list.size() == 1)
    {
        m_lineEdit = m_lineEdit.mid(0, m_linePos) + p_list.first() + m_lineEdit.mid(m_linePos);
...
{
    m_showInputLine = true;
    m_prompt = p_prompt + " ";
    rebuildPromptText();
    repaint();
}

bool ConsoleWidget::event(QEvent *p_event)
{
    if(p_event->type() == QEvent::KeyPress)
    {
        QKeyEvent *t_event = static_cast<QKeyEvent*>(p_event);
        if(t_event->key() == Qt::Key_Tab)
        {
            keyPressEvent(t_event);
            return true;
        }
    }
...
void ConsoleWidget::keyPressEvent(QKeyEvent *p_event)
{
    ...
    if(p_event->modifiers() == Qt::ControlModifier)
    {
        if(p_event->key() == Qt::Key_C)
        {
            ...
            for(int i = m_selection.fromRow; i <= m_selection.toRow; i++)
            {
                ...
                if(m_selection.toRow == i)
                    t_toCopy += joinLineText(m_displayedText[i]).mid(t_fromChar, m_selection.toChar - t_fromChar) + "\n";
                else
        ...
        else if(p_event->key() == Qt::Key_V)
        {
            ...
            for(QString it : t_clipBoard.split('\n'))
            {
                m_lineEdit.replace("\\\n", "\n");
                m_lineEdit = m_lineEdit.mid(0, m_linePos) + it + m_lineEdit.mid(m_linePos);
                m_lineEdit.replace('\n', "\\\n");
                m_linePos += it.size();
    ...
    {
        m_lineEdit.replace("\\\n", "\n");
        m_lineEdit = m_lineEdit.mid(0, m_linePos) + p_event->text() + m_lineEdit.mid(m_linePos);
        m_lineEdit.replace('\n', "\\\n");
        m_linePos++;
        m_scrollDown = true;
    }
...
void ConsoleWidget::mousePressEvent(QMouseEvent *p_event)
{
    ...
    m_selection.startingRow = m_selection.fromRow = m_selection.toRow = t_presumedRow;
    m_selection.startingChar = m_selection.fromChar = m_selection.toChar = t_presumedColumn;
}
...
void ConsoleWidget::mouseMoveEvent(QMouseEvent *p_event)
{
    ...
    else if(m_selection.startingRow == t_presumedRow && m_selection.startingChar > t_presumedColumn)
    {
        m_selection.fromChar = t_presumedColumn;
        m_selection.toChar = m_selection.startingChar;
    }
    ...
    else if(m_selection.fromRow >= m_displayedText.size() - 1) // It does need the equal, because of fromChar
    {
        m_selection.fromRow = m_displayedText.size() - 1;
        if(m_displayedText.size() != 0 && m_selection.fromChar >= lineLength(m_displayedText.last()) - 1)
    ...
    else if(m_selection.toRow >= m_displayedText.size() - 1)
    {
        ...
        if(m_displayedText.size() != 0 && m_selection.toChar >= lineLength(m_displayedText.last()) - 1)
        ...
        for(TextPart it : it_textparts)
        {
            ...
            if(t_rownum >= m_selection.fromRow && t_rownum <= m_selection.toRow)
            {
                ...
                if(t_rownum >= m_contentLines && m_paintCursor)
                {
                    ...
                    {
                        t_painter.drawText(QRectF(0, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(0, t_correctedLinePos - t_promptPos));
                        t_painter.setPen(t_highlightpen);
                        t_painter.setPen(t_highlightpen);
                        t_backgroundBrush.setColor(Qt::white);
                        t_painter.setBackground(t_backgroundBrush);
                        t_painter.setBackgroundMode(Qt::OpaqueMode);
                        t_painter.drawText(QRectF((t_correctedLinePos - t_promptPos) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_correctedLinePos - t_promptPos, 1));
                        t_painter.setPen(t_pen);
                        t_painter.setBackgroundMode(Qt::TransparentMode);
                        t_painter.drawText(QRectF((t_correctedLinePos - t_promptPos + 1) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_correctedLinePos - t_promptPos + 1, t_inlinePosFrom));
                    }
                    else
                        t_painter.drawText(QRectF(0, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(0, t_inlinePosFrom));

                    /// FROM t_inlinePosFrom to selection end
                    if(t_correctedLinePos - t_promptPos >= t_inlinePosFrom && t_correctedLinePos - t_promptPos < t_inlinePosFrom + t_inlineLength)
                    {
                        t_painter.setPen(t_highlightpen);
                        t_backgroundBrush.setColor(t_pen.color());
                        t_painter.setBackground(t_backgroundBrush);
                        t_painter.setBackgroundMode(Qt::OpaqueMode);
                        t_painter.drawText(QRectF(t_inlinePosFrom * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_inlinePosFrom, t_correctedLinePos - t_promptPos - t_inlinePosFrom));
                        t_painter.setPen(t_pen);
                        t_painter.setBackgroundMode(Qt::TransparentMode);
                        t_painter.drawText(QRectF((t_correctedLinePos - t_promptPos) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_correctedLinePos - t_promptPos, 1));
                        t_painter.setPen(t_highlightpen);
                        t_painter.setBackgroundMode(Qt::OpaqueMode);
                        t_painter.drawText(QRectF((t_correctedLinePos - t_promptPos + 1) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_correctedLinePos - t_promptPos + 1, t_inlinePosFrom + t_inlineLength - (t_correctedLinePos - t_promptPos + 1)));
                    }
                    else
                    {
                        t_painter.setPen(t_highlightpen);
                        ...
                        t_painter.setBackground(t_backgroundBrush);
                        t_painter.setBackgroundMode(Qt::OpaqueMode);
                        t_painter.drawText(QRectF(t_inlinePosFrom * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_inlinePosFrom, t_inlineLength));
                    }

                    /// FROM selection end to end of line
                    if(t_correctedLinePos - t_promptPos >= t_inlinePosFrom + t_inlineLength)
                    {
                        t_painter.setPen(t_pen);
                        t_painter.setBackgroundMode(Qt::TransparentMode);
                        t_painter.drawText(QRectF((t_inlinePosFrom + t_inlineLength) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_inlinePosFrom + t_inlineLength, t_correctedLinePos - (t_inlinePosFrom + t_inlineLength)));
                        t_painter.setPen(t_highlightpen);
                        t_backgroundBrush.setColor(Qt::white);
                        t_painter.setBackground(t_backgroundBrush);
                        t_painter.setBackgroundMode(Qt::OpaqueMode);
                        ...
                        t_painter.setPen(t_pen);
                        t_painter.setBackgroundMode(Qt::TransparentMode);
                        t_painter.drawText(QRectF((t_correctedLinePos - t_promptPos + 1) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_correctedLinePos - t_promptPos + 1));
                    }
                    else
                    {
                        t_painter.setPen(t_pen);
                        t_painter.setBackgroundMode(Qt::TransparentMode);
                        t_painter.drawText(QRectF((t_inlinePosFrom + t_inlineLength) * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(t_inlinePosFrom + t_inlineLength));
                    }
                }
                else
                {
            ...
            {
                if(t_rownum >= m_contentLines && m_paintCursor && m_linePos + m_prompt.size() >= t_promptPos && m_linePos + m_prompt.size() < t_promptPos + t_charperline)
                {
                    t_painter.drawText(QRectF(0, t_actualheight, width(), t_actualheight + m_charHeight), it.text.mid(0, t_correctedLinePos - t_promptPos));
                    t_painter.setPen(t_highlightpen);
                    t_backgroundBrush.setColor(Qt::white);
                    ...
                    t_painter.drawText(QRectF(t_inlinePos * m_charWidth, t_actualheight, width(), t_actualheight + m_charHeight), it.text);
            }
...
void ConsoleWidget::rebuildDisplayedText()
{
    ...
    {
        QColor t_color;
        if(it_line.type != LineType::Comment && m_lastLineType == LineType::Comment)
        {
            m_displayedText.push_back(QVector<TextPart>() << TextPart(" */", Qt::darkGreen));
        }
        ...
        for(QString it : it_line.content.split('\n'))
        {
            ...
            while(t_pos < it.size())
            {
                ...
                m_displayedText.push_back(QVector<TextPart>() << TextPart(t_part, t_color));
...
void ConsoleWidget::rebuildPromptText()
{
    ...
    if(m_showInputLine)
    {
        ...
        while(t_pos < t_prompt.size())
        {
            ...
            m_displayedText.push_back(QVector<TextPart>() << TextPart(t_part, Qt::white));
            t_pos += t_part.length();
