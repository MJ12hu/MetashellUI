...
MetashellInterpreter::MetashellInterpreter(bool p_isPlugin, QObject *parent) :
    QObject(parent),
    m_isPlugin(p_isPlugin)
{
    m_restartImmediately = false;
...
void MetashellInterpreter::setExecutable(const QString p_path)
{
    m_executable = p_path;
}

void MetashellInterpreter::start()
{
    if(m_metashell->state() != QProcess::ProcessState::NotRunning)
    {
        m_restartImmediately = true;
        m_metashell->terminate();
    }
    else
        m_metashell->start(m_executable, QStringList() << "--console=json");
}

void MetashellInterpreter::sendCommand(QString p_command)
{
    QJsonObject t_json;
    t_json["type"] = QString("cmd");
    t_json["cmd"] = p_command;
    m_metashell->write(QJsonDocument(t_json).toJson(QJsonDocument::Compact) + "\n");
...
{
    QJsonObject t_json;
    t_json["type"] = QString("code_completion");
    t_json["code"] = p_cc;
    m_metashell->write(QJsonDocument(t_json).toJson(QJsonDocument::Compact) + "\n");
    m_metashell->waitForBytesWritten();
}
...
{
    QJsonDocument t_json = QJsonDocument::fromJson(p_input.replace('\r', ""));
    if(t_json.isObject())
    {
        ...
        else if(t_type == "backtrace")
        {
            ...
            for(QJsonValue it : t_object["frames"].toArray())
            {
                ...
                if(t_frameObject.keys().contains("kind"))
                    t_frame.kind = t_frameObject["kind"].toString();
                if(t_frameObject.keys().contains("time_taken"))
                    t_frame.timeTaken = t_frameObject["time_taken"].toDouble();
                if(t_frameObject.keys().contains("time_taken_ratio"))
                    t_frame.timeTakenRatio = t_frameObject["time_taken_ratio"].toDouble();
        ...
        else if(t_type == "filename_list")
        {
            QStringList t_list;
            for(QJsonValue it : t_object["filenames"].toArray())
            {
                t_list.push_back(it.toString());
            }
            fileNameListArrived(t_list);
        }
        else if(t_type == "filename_set")
        {
            QStringList t_list;
            for(QJsonValue it : t_object["filenames"].toArray())
            {
...
{
    m_buffer += m_metashell->readAll();
    if(m_buffer.contains('\n'))
    {
        QList<QByteArray> t_input = m_buffer.split('\n');
        m_buffer.clear();
